    ...
    <>
      <div className="px-8 pb-8 text-lg border-b-8 border-solid border-[#eeeeee]">
        <div className=" border border-black-1 border-solid px-8 pt-6 pb-5">
          ...
          <div className=" my-4">
            Files can be uploaded and decentralized pinned to IPFS by using this web interface, or by CLI as shown in the curl sample below.
          </div>
...
export const Bucket = React.memo(() => {
  ...
  useOnce(() => {
    getLocalFileListByBucketId(bucketId)
      .then((res) => {
    ...
    getAuth(tokenId)
      .then(async (auth) => {
        try {
          ...
          setLocalFileList(
            localFileList.concat([{ name, cid, fileSize, fileType, createTime: moment().format("X").valueOf(), isNew: true }])
          );
        } catch (e) {
  ...
  return (
    <MainLayout
      menuId={1}
      tit={
        <div className=" bg-white py-3 flex items-center mr-auto">
    ...
    >
      <div className="flex-1 w-full pt-8 h-full overflow-y-auto">
        <div className="relative w-full">
          <TopInfo />
          <div className="px-8 flex-1 text-lg w-full flex flex-col">
            <div className="sticky pt-8 -top-8 bg-white w-full flex items-center z-10">
              <DropDownBtn
                dropData={[
                  { text: "File", icon: FiFile, value: "file" },
                  { text: "Folder", icon: FiFolder, value: "folder" },
                ]}
                text="Upload"
              ...
              <div className="relative w-1/2 h-12 max-w-sm border-solid border-black-1 border rounded overflow-hidden">
                <input
                  className="w-full h-full pl-5 pr-10 active:border-0"
                  onKeyDown={onInputKeyDownSearch}
                  onChange={(v) => setFilterText(v.target.value)}
                />
                <Icon icon={FiSearch} className="text-2xl absolute top-[0.7rem] right-2 cursor-pointer" onClick={doSearch} />
              </div>
            </div>
            <div className="w-full overflow-x-auto">
              <div className=" w-full min-w-[64rem] py-4 flex items-center font-medium border-b-1 border-solid border-b-black-1 pt-5">
              ...
              <div className=" min-w-[64rem] text-sm text-gray-6 w-full">
                {fFiles &&
                  fFiles[pgNum - 1] &&
                  fFiles[pgNum - 1].map((v, index) => (
                    <div
                      key={`files_${index}`}
                      className={classnames("flex items-center pt-4 pb-8 w-full", v.isNew ? "text-gray-300" : "")}
                    >
                      <div className="flex-initial w-[25%] md:w-[20%] pl-3">
                        <div className="flex items-center pr-8">
                          <span className="truncate" data-tip={v.name.length > 20 ? v.name : ""}>
                            {v.name}
                          </span>
                          {v.fileType === 1 && <Icon className="ml-2 min-w-[14px]" icon={FiFolder} />}
                        </div>
                      </div>
                      <div className="flex-initial w-[20%] md:w-[25%]">
                        <span data-tip={v.cid} data-for="cidColumn">
                          {shortStr(v.cid, 10, 10)}
                        </span>
                      </div>
                      <div
                        className="flex-initial w-[30%] truncate pr-8"
                        data-for="linkColumn"
                        data-tip={`${GatewayBaseBucket}/ipfs/${v.cid}`}
                      >{`${GatewayBaseBucket}/ipfs/${v.cid}`}</div>
                      <div className="flex-initial w-[10%]">{formatFileSize(v.fileSize)}</div>
                      <div className="flex-initial w-[15%] text-gray-6">
                        {v.isNew ? (
                          <span
                            data-tip={`The ${
                              v.fileType === 0 ? "file" : "folder"
                            } has been successfully uploaded to your bucket. It takes several minutes to finalize the decentralized storage and IPNS update processes.`}
                          >
                            <Icon icon={BsQuestionCircle} />
                          </span>
                        ) : (
                          moment(v.createTime * 1000).format("YYYY-MM-DD HH:mm:ss")
                        )}
                      </div>
                    </div>
                  ))}
      ...
      {upState.status !== "stop" && (
        <Modal>
          ...
          <div className="bg-white mt-5 flex  py-3 cursor-pointer justify-between items-center h-20">
            {upState.status === "upload" && <ProgressBar value={upState.progress} />}
            {upState.status === "success" && <Alert text={"Upload successful"} status={upState.status} />}
            {upState.status === "fail" && <Alert text={upState.errorMsg ? upState.errorMsg : "Upload fail"} status={upState.status} />}
            {upState.status === "cancel" && <Alert text={"Upload cancel"} status={"fail"} />}
          </div>
        </Modal>
      )}
      <ReactTooltip
