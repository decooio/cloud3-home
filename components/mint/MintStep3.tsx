...
function TulpeText(p: { data: [string, string] | [string, string, string]; target?: "_blank" | "_self" }) {
  ...
  return (
    <div className=" flex items-center text-2xl">
      ...
      {link ? (
        <a className=" underline underline-offset-2 !text-black-1" target={target} href={link}>
          {value}
        </a>
      ) : (
...
export const MintStep3 = React.memo((p: MintStep3Props) => {
  ...
  const currentEdition = useMemo(() => editions.find((item) => item.id === currentEditionId), [editions, currentEditionId]);
  ...
  const doMint = useOn(async () => {
    if (!minting && isAlgoConnected && mintData.price && mintData.editionId) {
      setMinting(true);
      try {
        ...
        const value = ethers.utils.parseUnits(mintData.price.fmtPrice, mintData.price.decimals);
        const paymentTxn = (() => {
        ...
        const applicationCallObject = {
          ...
          methodArgs: [paymentTxnWithSigner, mintData.editionId, getSHA256Digest(mintData.metadata), `ipfs://${mintData.metadataCID}#arc3`],
    ...
    } else if (!minting && w3b && address && mintData.price && mintData.editionId && writeContractAsync && pc) {
      setMinting(true);
      const value = parseUnits(mintData.price.fmtPrice, mintData.price.decimals);
      try {
        const isEth = mintData.price.currency === "0x0000000000000000000000000000000000000000";
      ...
      {!minting && !mintData.mintTx && (
        <div className="flex flex-1 px-12 flex-col items-center justify-center">
          <div className=" text-2xl text-center">
            Choose your preferred payment method and click the ‘Confirm and Pay’ button to proceed:
          </div>
          <div className=" flex ">
            {currentEdition.prices.map((price, index) => (
              <div key={`edition_price_${index}`} className={classNames("mr-4 text-center")}>
                <Button
                  text={price.symbol}
                  disHover={price.currency === mintData.price?.currency}
                  className={classNames("font-medium !border-2 text-2xl mt-5 !w-[11.25rem] !py-[9px] cursor-pointer", {
                    "!border-orange-15 !text-orange-15": price.currency === mintData.price?.currency,
                  })}
                  onClick={() => {
      ...
      {!minting && mintData.mintTx && (
        <div className="flex flex-1 px-12 flex-col ">
          <div className=" text-2xl font-medium mb-8">
            Congrats,
            <br />
            You have completed all the minting processes for this W3Bucket NFT!
          </div>
          <TulpeText
            data={[
          ...
          <div className=" text-2xl font-medium mt-8">Return to the W3Bucket Home Page and start your Crust Cloud journey. Bon Voyage!</div>
          <Button text="Complete" className=" self-center text-lg !w-[11.25rem] h-[3.375rem] mt-20" onClick={onComplete} />
        </div>
      )}
    </div>
  );
